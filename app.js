...
/* ---------------- Storage katmanı ----------------
   PAYLAŞIMLI (herkes aynı veriyi görür) — Supabase.
   Tabloların yapısı:
     app_state(id int primary key, data jsonb, updated_at timestamptz)
     player_photos(player_id text primary key, photo text, updated_at timestamptz)
   Kurulum SQL'i ve talimatlar index.html içinde yorum olarak var. */
const sb = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);

let supabaseReady = false;
let saving = false;
let suppressAutoRender = false; // form doldururken uzaktan gelen güncelleme üzerine yazmasın
...
async function saveState() {
  ...
  try {
    const { error } = await sb.from('app_state').upsert({
      id: 1, data: coreStateSnapshot(), updated_at: new Date().toISOString()
    });
...
async function savePhotoToServer(playerId, dataUrl) {
  ...
  try {
    const { error } = await sb.from('player_photos').upsert({
      player_id: playerId, photo: dataUrl, updated_at: new Date().toISOString()
    }, { onConflict: 'player_id' });
    if (error) throw error;
    photoTableError = null;
  } catch (e) {
    console.error('Fotoğraf kaydedilemedi:', e);
    photoTableError = (e && (e.message || e.details || e.hint)) || 'bilinmeyen hata';
    toast('Fotoğraf kaydedilemedi: ' + photoTableError, 6000);
  }
}
...
async function deletePhotoFromServer(playerId) {
  if (!supabaseReady) return;
  try {
    await sb.from('player_photos').delete().eq('player_id', playerId);
  } catch (e) { /* zaten yoksa sorun değil */ }
}

let photoTableError = null;

async function loadAllPhotosOnce() {
  if (!supabaseReady) return;
  try {
    const { data, error } = await sb.from('player_photos').select('player_id, photo');
    if (error) throw error;
    photoTableError = null;
    (data || []).forEach(row => {
...
async function refreshFromServer() {
  ...
  try {
    ...
    if (!error && data) applyLoadedCoreState(data.data);
...
async function initApp() {
  try {
    ...
    if (data && data.data) {
      ...
      supabaseReady = true;
      await saveState(); // ilk çalıştırma: varsayılan 14 oyuncuyu sunucuya yaz
    }
    supabaseReady = true;
    await loadAllPhotosOnce();
    render();

    // Gerçek zamanlı senkronizasyon: başka biri değişiklik yapınca herkes otomatik görsün
    ...
    sb.channel('photo_sync')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'player_photos' }, (payload) => {
        if (payload.eventType === 'DELETE') {
          const p = getPlayer(payload.old.player_id);
          if (p) p.photo = null;
        } else if (payload.new) {
          const p = getPlayer(payload.new.player_id);
          if (p) p.photo = payload.new.photo;
        }
        if (!suppressAutoRender) render();
      })
      .subscribe();
    ...
    showSupabaseSetupError(e);
  }
}

function showSupabaseSetupError(e) {
  ...
  app.innerHTML = `
    <div style="padding:40px 20px;text-align:center;color:#F4F7F2;">
      ...
      <p style="color:rgba(244,247,242,0.65);font-size:0.85rem;line-height:1.5;">
        index.html içindeki kurulum talimatlarını takip ederek Supabase projende
        <b>app_state</b> ve <b>player_photos</b> tablolarını oluşturduğundan emin ol.
      </p>
...
function toast(msg, durationMs) {
  ...
  toastTimer = setTimeout(() => el.remove(), durationMs || 2200);
}
...
function openEditPlayerModal(playerId) {
  ...
  openSheet(`
    ...
    ${photoTableError ? `<div style="background:rgba(228,72,63,0.1);color:var(--red-card);border-radius:10px;padding:10px 12px;font-size:0.75rem;margin-bottom:10px;line-height:1.4;">⚠️ Fotoğraf veritabanına bağlanılamadı: <b>${escapeHtml(photoTableError)}</b><br>Supabase'de <code>player_photos</code> tablosunu ve izinlerini (RLS policy) kontrol et — index.html içindeki kurulum SQL'ine bak.</div>` : ''}
    ...
    <label>İsim</label><input id="editPlayerName" type="text" value="${escapeHtml(p.name)}">
    <label style="margin-top:10px;display:block;">Numara</label><input id="editPlayerNumber" type="number" value="${p.squadNumber}">
...
function savePlayerEdit(playerId) {
  ...
  const number = Number(document.getElementById('editPlayerNumber')?.value) || 1;
  if (!name) return toast('İsim boş olamaz');
  requireAuth(() => {
    const p = getPlayer(playerId);
    p.name = name;
    p.squadNumber = number;
    saveState();
    closeSheet();
    renderPlayers();
    toast('Güncellendi');
...
async function handlePhotoUpload(event, playerId) {
  ...
  try {
    ...
    if (!photoTableError) {
      ...
      openEditPlayerModal(playerId); // hata banner'ını göstermek için tekrar aç
    }
  } catch (e) {
    console.error(e);
    ...
    <div class="page">
      ...
      <div class="card" style="text-align:center;">
        <div style="font-size:0.85rem;color:var(--ink-soft);margin-bottom:10px;">
          İstatistik, fotoğraf, isim ve forma numarası değiştirme ${isUnlocked ? '<b style="color:var(--pitch);">şu an açık 🔓</b>' : '<b>şifreyle korunuyor 🔒</b>'}
        </div>
...
function renderTeamOfWeek(param) {
  const week = param ? getWeek(param) : latestWeek();
  const app = document.getElementById('app');
  if (!week) {
    app.innerHTML = `${topbarHTML("Haftanın 6'sı")} <div class="page"><div class="empty-state"><h3>Maç bulunamadı</h3></div></div>`;
    return;
  }
  app.innerHTML = `${topbarHTML("Haftanın 6'sı")} <div class="page"><div class="card">Hafta ${week.weekNumber} kadro özeti</div></div>`;
}